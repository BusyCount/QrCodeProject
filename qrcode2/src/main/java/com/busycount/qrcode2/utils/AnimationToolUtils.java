package com.busycount.qrcode2.utils;
...
//                textView.setTextColor((Integer) animation.getAnimatedValue());
    ...
    public static void cardFilpAnimation(final View beforeView, final View AfterView) {
        ...
        if (beforeView.getVisibility() == View.GONE) {
            ...
            ImageToolUtils.invisToVis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", -90f, 0f);
            ImageToolUtils.visToInvis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", 0f, 90f);
        } else if (AfterView.getVisibility() == View.GONE) {
            ImageToolUtils.invisToVis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", -90f, 0f);
            ImageToolUtils.visToInvis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", 0f, 90f);
        }

        ImageToolUtils.visToInvis.setDuration(250);// 翻转速度
        ImageToolUtils.visToInvis.setInterpolator(accelerator);// 在动画开始的地方速率改变比较慢，然后开始加速
        ImageToolUtils.invisToVis.setDuration(250);
        ImageToolUtils.invisToVis.setInterpolator(decelerator);
        ImageToolUtils.visToInvis.addListener(new Animator.AnimatorListener() {

            @Override
            public void onAnimationEnd(Animator arg0) {
                if (beforeView.getVisibility() == View.GONE) {
                    AfterView.setVisibility(View.GONE);
                    ImageToolUtils.invisToVis.start();
                    beforeView.setVisibility(View.VISIBLE);
                } else {
                    AfterView.setVisibility(View.GONE);
                    ImageToolUtils.visToInvis.start();
                    beforeView.setVisibility(View.VISIBLE);
                }
            }
